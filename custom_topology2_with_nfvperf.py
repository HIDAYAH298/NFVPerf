...
def customTopo():
    ...
    # Install and configure NFVPerf on hosts
    setup_nfvperf_on_host(h1, '10.0.0.2')
    setup_nfvperf_on_host(h2, '10.0.0.1')
...
def setup_nfvperf_on_host(host, target_ip):
...
"""
    ...
    host.cmd(f'nfvperf --config {config_path} --host {host.name} --target {target_ip} &')

    # Ping the switch (s1)
    host.cmd(f'ping -c 4 {host.defaultIntf().IP()}')
